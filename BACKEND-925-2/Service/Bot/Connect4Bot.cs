...
{
    public class Connect4Bot(string difficulty, Guid gameStateId, Player2PlayerGame player, GamesDbContext gameDbContext) : IBot
    {
        private readonly Connect4Service gameService = new(gameStateId, player, Player2PlayerGame.Null(), new Connect4Repository(gameDbContext));
        private readonly Player2PlayerGame player = player;
        ...
        public static int Score_Connect_4_Block(List<IPiece> connect4Block, Player2PlayerGame currentPlayer, Player2PlayerGame currentOpponent)
        {
            int score = 0;
            int playerPieces = 0;
        ...
        private int ScoreBoard(Player2PlayerGame currentPlayer, Player2PlayerGame currentOpponent)
        {
            int score = 0;
        ...
        public (int, int) GetBestMove(int? searchDepth = null, bool isMaximizingPlayer = true)
        {
            ...
            if (searchDepth == 0 || IsTerminalNode())
            {
                if (searchDepth == 0)
                {
                    Player2PlayerGame? currentOpponent;
                    Player2PlayerGame? currentPlayer;
                    if (isMaximizingPlayer)
                    {
                        currentPlayer = Player2PlayerGame.Null();
                        currentOpponent = player;
                    }
                    else
                    {
                        currentOpponent = Player2PlayerGame.Null();
                ...
                {
                    if (gameService.GetGame().GameState.Winner == Player2PlayerGame.Null())
                    {
                ...
                if (isMaximizingPlayer)
                {
                    ...
                    while (check < 7)
                    {
                        ...
                        if (ints.Contains(randInt))
                        {
                            ...
                            {
                                IPiece piece = gameService.DropPiece(randInt, Player2PlayerGame.Null());
