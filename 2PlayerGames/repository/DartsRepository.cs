...
namespace TwoPlayerGames.Repo
{
    public class DartsRepository : BaseGameRepository
    {
        ...
        public override Dictionary<Guid, IGame> GetGames()
        {
            ...
            using (SqlCommand command = new SqlCommand("SELECT * FROM getAllDartsGames", sqlConnection))
            {
                sqlConnection.Open();
                using (SqlDataReader reader = command.ExecuteReader())
                {
                    while (reader.Read())
                    {
                        ...
                        GameState gameState = new (gameStateId, player1, player2, gameType, turn, timePlayed, winner, jsonString);
